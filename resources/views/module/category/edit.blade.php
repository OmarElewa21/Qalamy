    ...
    <form action="{{route('categories.update')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input value="{{ $category->icon }}" name="icon" class="icon" type="hidden">
                ...
                @if (MediaActive())
                    {{-- media --}}
                    <a href="javascript:void()" onclick="openNav('{{ route('media.slide') }}', 'category')" class="btn btn-primary media-btn mt-2 p-2">@translate(Upload From Media) <i class="fa fa-cloud-upload ml-2" aria-hidden="true"></i> </a>
                @endif
