...
<div class="box" dir="{{(App::isLocale('ar') ? 'rtl' : 'ltr')}}">

		<div class="box-header with-border">
			<div class="box-title" style="{{(App::isLocale('ar') ? 'text-align:right' : '')}}">
				@translate(Create a new live class)
			</div>
		</div>
		...
		<div class="box-body">
			<form autocomplete="off" action="{{ route('zoom.store') }}" method="POST">
					...
					<div class="form-group">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}" for="exampleInputDetails">@translate(Select Course:)</label>
					<br>
								<input type="checkbox"
					...
					<div class="form-group">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">
							@translate(Meeting Topic:)<sup class="text-danger">*</sup>
						</label>
					...
					<div class="custom-checkbox">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}" for="recurring_meeting">@translate(Recurring Metting (with no fixed time)) </label>
						<br>
					...
					<div class="form-group" id="sec4_four">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">
							@translate(Meeting Start Time:)<sup class="text-danger">*</sup>
						</label>
					...
					<div class="form-group" id="sec3_three">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">
							@translate(Duration:)<sup class="text-danger">*</sup>
						</label>

						<input placeholder="@translate(Enter in minutes)" type="number" min="1" name="duration" class="form-control">
						<small>@translate(For example: 45)</small>
					</div>

					<div class="form-group">
						<div class="eyeCy">
							<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">
								@translate(Meeting Password: (Optional))
							</label>
					...
					<div class="form-group">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">
							@translate(Meeting Agenda:)<sup class="text-danger">*</sup>
						</label>
					...
					<div class="form-group">
						<label style="{{(App::isLocale('ar') ? 'float:right' : '')}}">@translate(TimeZone:)</label>
						...
						<select class="form-control lang js-example-basic-single" name="timezone">
							...
							<small class="text-muted"><i class="fa fa-question-circle"></i> @translate(Set to None if you want to use your account timezone.)</small>
					</div>

					<div class="panel panel-default">
						<div class="panel-body">
							<h5 style="{{(App::isLocale('ar') ? 'text-align:right' : '')}}" class="panel-title">@translate(Meeting Setting :)</h5>
							<h>
							<div class="custom-control custom-checkbox">
							  <input name="host_video" type="checkbox" class="custom-control-input" id="host_video">
							  <label class="custom-control-label" for="host_video">@translate(Enable Host Video)</label>
							</div>

							<div class="custom-control custom-checkbox">
							  <input name="participant_video" type="checkbox" class="custom-control-input" id="participant_video">
							  <label class="custom-control-label" for="participant_video">@translate(Enable Participant Video)</label>
							</div>

							<div class="custom-control custom-checkbox">
							  <input name="join_before_host" type="checkbox" class="custom-control-input" id="join_before_host">
							  <label class="custom-control-label" for="join_before_host">@translate(Join before host?)</label>
							</div>

							<div class="custom-control custom-checkbox">
							  <input name="mute_upon_entry" type="checkbox" class="custom-control-input" id="mute_upon_entry">
							  <label class="custom-control-label" for="mute_upon_entry">@translate(Mute Upon Entry?)</label>
							</div>
					...
					<hr>
					<div class="m-1 form-group" style="{{(App::isLocale('ar') ? 'text-align:right' : '')}}">
							<button class="btn btn-success btn-md">@translate(Create Meeting)</button>
						</div>
