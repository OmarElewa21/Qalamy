...
<div class="container-fluid shadow">
    <form action="{{ route('media.store') }}" method="POST" enctype="multipart/form-data" id="media-form">
        ...
        <div class="row">
            <div class="col-8">
                <label for="">@translate(Upload File)</label>
                <div class="card shadow-sm bg-dark text-white">
                    <input type="file" value="{{ old('image') }}" name="image" class="dropify media-img">
                </div>
                <p class="Error text-danger"></p>
            <small>@translate(Maximum upload size 2GB)</small>
            <br>
            <small>@translate(Supported file type: jpeg, jpg, png, pdf, mp4, zip)</small>
            </div>
          <div class="col-4">

                <div class="form-group">
                    <label for="title">@translate(Title (optional))</label>
                    <input type="text" class="form-control" name="title" id="title" value="{{ old('title') }}" aria-describedby="emailHelp" placeholder="@translate(Enter title)">
                </div>


                {{-- Type --}}
                <div class="form-group" dir="{{(App::isLocale('ar') ? 'rtl' : 'ltr')}}">
                        <label for="type">
